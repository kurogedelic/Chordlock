...
ChordNameResult ChordNameGenerator::generateChordName(const ChordMatch& match,
    ...
    // Step 1: Use root from ChordMatch if available, otherwise detect
    ...
    result.is_slash_chord = bass_differs_from_root && 
                           shouldUseSlashNotation(match.chord_info.name, result.inversion_type, 
                                                match.bass_note_midi - theoretical_root);
    
    // Step 6: Handle bass note for slash chords
    if (result.is_slash_chord && match.bass_note_midi != -1) {
        result.bass_note = note_converter->midiToNoteName(match.bass_note_midi, accidental_style, OctaveNotation::NO_OCTAVE);
    }
...
bool ChordNameGenerator::shouldUseSlashNotation(const std::string& chord_type, int inversion, int bass_interval) const {
    // Always use slash notation for non-chord tones in bass
    if (bass_interval != 0 && bass_interval != 4 && bass_interval != 7 && bass_interval != 10) {
        return true;
    }
    
    // Use slash notation for clear inversions of triads
